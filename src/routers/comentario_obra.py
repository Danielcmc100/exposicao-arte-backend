...
from models.comentario_obra import ComentarioObraCreate, ComentarioObraDB, ComentarioObraResponse
...
def ler_comentario_obra(comentario_obra_id: int, session: SessionInjetada) -> ComentarioObraResponse | None:
    comentario_obra = buscar_comentario_obra_por_id(comentario_obra_id, session)
    return ComentarioObraResponse.model_validate(comentario_obra) if comentario_obra else None


@rota.post("/")
def criar_comentario_obra(comentario_obra: ComentarioObraCreate, session: SessionInjetada) -> ComentarioObraResponse:
    comentario_obra_db = ComentarioObraDB.model_validate(comentario_obra)
    return ComentarioObraResponse.model_validate(adicionar_comentario_obra(comentario_obra_db, session))


@rota.put("/{comentario_obra_id}")
def atualizar_comentario_obra(
    comentario_obra_id: int, comentario_obra: ComentarioObraCreate, session: SessionInjetada
) -> ComentarioObraResponse | None:
    comentario_obra_db = ComentarioObraDB.model_validate(comentario_obra)
    comentario_obra_atualizado = atualizar_comentario_obra_bd(comentario_obra_id, comentario_obra_db, session)
    return ComentarioObraResponse.model_validate(comentario_obra_atualizado) if comentario_obra_atualizado else None
