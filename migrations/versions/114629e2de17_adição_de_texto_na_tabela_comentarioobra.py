...
def upgrade() -> None:
    ...
    # Criar ENUM type primeiro (PostgreSQL) usando checkfirst
    status_enum = postgresql.ENUM('ATIVO', 'INATIVO', 'PENDENTE', 
                                   name='statuscomentario', 
                                   create_type=False)
    status_enum.create(op.get_bind(), checkfirst=True)
    
    # Criar tabela usando o ENUM já criado
    op.create_table(
        'comentario_eventos',
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    # Recriar colunas antigas usando batch mode
    with op.batch_alter_table('obras', recreate='always') as batch_op:
    ...
    # Drop ENUM type (PostgreSQL) usando checkfirst
    postgresql.ENUM(name='statuscomentario').drop(op.get_bind(), checkfirst=True)
    # ### end Alembic commands ###
